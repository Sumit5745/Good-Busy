...
try {
  connectDB();
  logger.info("Database connected successfully", { service: "user-ms" });
} catch (error: any) {
...
app.listen(envConfig.services["user-ms"].PORT, () => {
  logger.info(
    `User microservice is running on port ${envConfig.services["user-ms"].PORT}`,
    {
      service: "user-ms",
      port: envConfig.services["user-ms"].PORT,
    }
  );
});
