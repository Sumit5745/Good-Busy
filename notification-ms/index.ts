...
app.listen(envConfig.services["notification-ms"].PORT, () => {
  logger.info(
    `Notification microservice is running on port ${envConfig.services["notification-ms"].PORT}`,
    {
      service: "notification-ms",
      port: envConfig.services["notification-ms"].PORT,
    }
  );
});
